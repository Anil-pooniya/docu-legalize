...
import { FileText, DownloadIcon, PrinterIcon, CheckCircleIcon, Loader2 } from "lucide-react";
...
const DocumentPreview: React.FC<DocumentPreviewProps> = ({ documentId = "1" }) => {
  ...
  const handleGenerateCertificate = () => {
    ...
    certificateMutation.mutate(documentData.id, {
      onSuccess: (data) => {
        generateAndDownloadCertificate(documentData, data.certificateId);
      }
    });
  };

  const generateAndDownloadCertificate = (doc: any, certificateId: string) => {
...
2. The electronic document titled "${doc.name}" was produced from a computer system which:
...
This certificate is issued in accordance with Section 65B of the Indian Evidence Act, 1872, and serves as legal attestation for the admissibility of the electronic document as evidence in legal proceedings.
    ...
    element.download = `Section65B_Certificate_${certificateId}.txt`;
